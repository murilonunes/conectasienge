...
export function LoginForm({ next, invalid, blocked }: { next: string; invalid?: boolean; blocked?: boolean }) {
  ...
  const [message, setMessage] = useState(blocked ? "Muitas tentativas. Aguarde alguns minutos." : invalid ? "Senha inválida. Tente novamente." : "");
  ...
  return (
    <form onSubmit={submit} className="auth-login-form">
      <label>
        <span>Senha</span>
        <input autoFocus value={password} onChange={(event) => setPassword(event.target.value)} type="password" required />
      </label>
