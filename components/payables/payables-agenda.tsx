      ...
      {buckets.map((bucket) => (
        <section className="card payables-period" key={bucket.id}>
          <div className="payables-period-head">
            ...
            <div><strong>{formatCurrency(bucket.amount)}</strong><span>{bucket.items.length} parcela{bucket.items.length === 1 ? "" : "s"}</span></div>
          </div>
              ...
              return (
                <article key={`${item.billId}-${item.installmentId}`}>
                  ...
                  <div>
                    ...
                    <span>{item.companyName || "Empresa não informada"} - {documentLabel(item)}</span>
                    <span>Título #{item.billId} / Parcela {item.installmentId}</span>
                    <span>Original {formatCurrency(review.originalAmount)} - Multa/juros pagos a mais {formatCurrency(review.paidIncrease)}</span>
