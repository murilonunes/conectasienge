  ...
  return (
    <section className="card panel sales-monthly-panel">
      <div className="panel-head">
        <div><h2 className="panel-title">Vendas por mês</h2><span className="panel-note">Valor de venda e quantidade de contratos emitidos</span></div>
        ...
        {data.map((item) => (
          <div className="sales-monthly-column" key={item.label} title={`${item.label}: ${formatCompactCurrency(item.value)} em ${item.count} contratos`}>
            ...
            <div className="sales-monthly-bars">
              <i className="sales-value-bar" style={{ height: `${Math.max(4, (item.value / maxValue) * 100)}%` }} />
              <i className="sales-count-bar" style={{ height: `${Math.max(4, (item.count / maxCount) * 100)}%` }}><b>{item.count}</b></i>
            </div>
