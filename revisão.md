...
| Revisado | Alta | `/kanban-compras` | `app/kanban-compras/page.tsx`, `components/purchases/purchase-project-kanban.tsx`, `features/purchases/project-kanban-data.ts`, `lib/purchase-project-kanban.ts`, `app/api/purchases/project-kanban/route.ts` | Criado Kanban gerencial com projetos/obras cadastrados localmente, fora do Sienge. Solicitações do espelho local podem ser vinculadas e desvinculadas em cada projeto, com vínculo único para impedir dupla contagem. Etapas são dinâmicas, ordenáveis e protegidas contra exclusão enquanto possuem projetos; o modal de etapas permite editar cada nome e confirmar pelo botão textual `Salvar nome` ou pela tecla Enter, sincronizando o valor retornado pelo servidor. Cartões podem ser movidos por drag-and-drop ou seletor. Durante o arraste, uma prévia acompanha o cursor e o marcador de inserção permite escolher a posição exata no início, no meio ou no fim de qualquer coluna, inclusive reordenar a etapa atual. A posição fica gravada no SQLite local; ao soltar, a interface muda imediatamente e desfaz o movimento se a gravação falhar. Ao clicar no cartão, abre uma visão gerencial ampla com etapa, prazo, cobertura, solicitações, itens e observações, além das cotações relacionadas com itens, fornecedores, respostas, propostas e menor valor, sem navegar para outras telas nem consultar o Sienge. A primeira solicitação e cotação começam expandidas e as demais podem ser abertas no próprio painel. Edição e vínculos ficam em modos separados; os controles de adicionar ou remover solicitações só aparecem depois do botão `Gerenciar vínculos`. O painel geral de pendências cruza o vínculo local cotação → solicitação com solicitação → projeto e permite resolver casos incompletos localmente. A tela possui busca, estados vazios, layout responsivo, i18n e permissão própria `screen.kanban-compras`; o conteúdo usa toda a largura disponível e o quadro acompanha a altura útil da janela, mantendo rolagem horizontal em telas menores. Nenhuma ação grava no Sienge. |
