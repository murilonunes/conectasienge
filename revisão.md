...
| Revisado | Alta | `/kanban-compras` | `app/kanban-compras/page.tsx`, `components/purchases/purchase-project-kanban.tsx`, `features/purchases/project-kanban-data.ts`, `lib/purchase-project-kanban.ts`, `app/api/purchases/project-kanban/route.ts` | Criado Kanban gerencial com projetos/obras cadastrados localmente, fora do Sienge. Solicitações do espelho local podem ser vinculadas e desvinculadas em cada projeto, com vínculo único para impedir dupla contagem. Etapas são dinâmicas e ordenáveis; o modal permite editar nomes e definir marcadores exclusivos `Inicial` e `Finalizada`, classificando projetos e solicitações como backlog, iniciados ou concluídos e impedindo uma ordem inválida. Cada solicitação vinculada possui etapa própria e usa as mesmas colunas do projeto; vínculos existentes são migrados para a etapa atual do projeto e novos entram na primeira coluna. Etapas marcadas ou com projetos/solicitações não podem ser excluídas. Cartões podem ser movidos por drag-and-drop ou seletor, com prévia junto ao cursor e posição exata persistida. Ao clicar no cartão, abre uma visão gerencial ampla com etapa, prazo, cobertura, totais por situação, solicitações, itens, observações e cotações relacionadas, sem navegar para outras telas nem consultar o Sienge. Edição e vínculos ficam em modos separados; os controles de adicionar, remover ou alterar a etapa de uma solicitação aparecem em `Gerenciar vínculos`. O painel geral de pendências cruza o vínculo local cotação → solicitação com solicitação → projeto e permite resolver casos incompletos localmente. A tela possui busca, estados vazios, layout responsivo, i18n e permissão própria `screen.kanban-compras`; o conteúdo usa toda a largura disponível e o quadro mantém rolagem horizontal em telas menores. Nenhuma ação grava no Sienge. |
