...
| Revisado | Alta | `/cotacoes` | `app/cotacoes/page.tsx`, `components/purchases/quotations-portal.tsx`, `components/purchases/quotations/*`, `app/api/sienge/purchase-quotations/route.ts` | Revisado: a tela lê o espelho local, mantém filtros/status/exportação CSV, permite selecionar quais itens da solicitação entram na cotação Sienge e chama o Sienge somente nos botões de preparo/confirmação de criação da cotação. |
