...
| Revisado | Alta | `/kanban-compras` | `app/kanban-compras/page.tsx`, `components/purchases/purchase-project-kanban.tsx`, `features/purchases/project-kanban-data.ts`, `lib/purchase-project-kanban.ts`, `app/api/purchases/project-kanban/route.ts` | Criado Kanban gerencial com projetos/obras cadastrados localmente, fora do Sienge. Solicitações do espelho local podem ser vinculadas e desvinculadas em cada projeto, com vínculo único para impedir dupla contagem. Etapas são dinâmicas, ordenáveis e protegidas contra exclusão enquanto possuem projetos; cartões podem ser movidos por drag-and-drop ou seletor. Durante o arraste, uma prévia acompanha o cursor e o marcador de inserção permite escolher a posição exata no início, no meio ou no fim de qualquer coluna, inclusive reordenar a etapa atual. A posição fica gravada no SQLite local; ao soltar, a interface muda imediatamente e desfaz o movimento se a gravação falhar. Projetos aceitam previsão de encerramento, com data no cartão e alerta de atraso. Cada solicitação exibe quantidade e números das cotações relacionadas. O painel de pendências cruza o vínculo local cotação → solicitação com solicitação → projeto, lista cotações sem origem ou fora do Kanban e permite resolver ambos os casos localmente. A tela possui busca, estados vazios, layout responsivo, i18n e permissão própria `screen.kanban-compras`; o conteúdo usa toda a largura disponível e o quadro acompanha a altura útil da janela, mantendo rolagem horizontal em telas menores. Nenhuma ação grava no Sienge. |
