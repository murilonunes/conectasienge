...
| Revisado | Alta | `/solicitacoes-compra` | `app/solicitacoes-compra/page.tsx`, `components/purchases/purchase-requests-portal.tsx` | A tela passou a ler a observação do cabeçalho sincronizado e a observação individual de cada insumo. Quando há conteúdo, um indicador com tooltip aparece na lista, no cabeçalho e junto ao item; especificação e observação permanecem separadas e ambas são preservadas no CSV. |
