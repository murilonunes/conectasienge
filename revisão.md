...
| Revisado | Média | `/contas-pagar` | `app/contas-pagar/page.tsx`, `features/payables-schedule/data.ts`, `features/payables-recurring/data.ts`, `components/payables/recurring-payables-panel.tsx` | Revisado: agenda segue lendo SQLite local e o atalho direto para `Novo lançamento` foi removido para não misturar consulta com escrita no Sienge. Adicionado painel "Recorrentes sem lançamento futuro" com seletor de janela (3/6/9/12 meses ou no ano) e botão de impressão isolada do painel, pra achar fornecedores com título todo mês (pago ou não) sem nenhum título futuro cadastrado. |
