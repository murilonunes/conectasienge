...
| --- | --- | --- | --- | --- |
| Revisado | Alta | Todas as telas | `app/layout.tsx`, `components/i18n/*`, `lib/i18n/*`, `components/ui/app-shell-client.tsx`, `scripts/mark-ui-text-for-i18n.mjs`, `scripts/audit-i18n.mjs` | Adicionado i18n em Português e Inglês com Português como padrão, seletor persistente por cookie no cabeçalho e nas telas públicas, idioma do documento/metadados e marcação dos textos estáticos em páginas, formulários, tabelas, modais e relatórios. O catálogo inglês cobre textos longos, mensagens operacionais, etapas, permissões, propriedades internas, frases parametrizadas com números/plurais/períodos e valores compactos. A auditoria percorre `.ts` e `.tsx`, bloqueando português residual, palavras mantidas indevidamente do texto-fonte e JSX sem marcação; termos técnicos e dados reais são preservados. Dados do Sienge e cadastros permanecem no idioma original. |
...
| Revisado | Alta | `/compras` | `app/compras/page.tsx`, `components/purchases/purchases-portal.tsx`, `features/purchases/supply-overview.ts`, `features/purchases/request-headers.ts` | Corrigida a aba Registros: deixou de receber só 500 itens e passou a buscar páginas filtradas em `/api/purchases/records`, lendo somente o SQLite local. Reformulada a visão geral como portal de suprimentos com cards, funil e painéis de ação. O filtro server-side mantém `Mês atual` ativo por padrão e aceita histórico adicional de +1, +2, +3, +6, +12 meses ou Tudo, aplicando o mesmo recorte a indicadores, ações, rankings e gráficos. Adicionada sincronização incremental da situação individual das solicitações via `/v1/purchase-requests/{id}` no job de Atualizar Compras, porque o espelho em lista não informa se a solicitação já foi atendida. |
| Revisado | Alta | `/configuracoes`, `/solicitacoes-compra`, `/cotacoes`, `/kanban-compras` | `lib/settings.ts`, `features/purchases/purchase-cutoff.ts`, `features/quotations/data.ts`, `features/purchases/project-kanban-data.ts`, `app/configuracoes/page.tsx` | Adicionada data de corte opcional para Compras. Registros anteriores à data ficam ocultos, a data informada permanece inclusa e campo vazio mostra todo o histórico. A regra compartilhada roda no servidor antes de listas, contagens e vínculos; solicitações manuais usam o mesmo corte no navegador e o Kanban reaplica o recorte após qualquer resposta da API. O filtro não apaga SQLite, projetos ou vínculos e preserva registros sem data confiável. |
| Revisado | Alta | `/rastreabilidade-insumos` | `app/rastreabilidade-insumos/page.tsx`, `components/purchases/resource-traceability.tsx`, `features/resource-traceability/data.ts` | Criada busca por código, descrição, sinônimo ou código auxiliar com histórico de solicitações, pedidos e notas fiscais do mesmo insumo. Usa relações exatas do dump local para mostrar SC → pedido → nota, valores, quantidades, fornecedor, obra e data da fonte, sem associar documentos apenas por texto semelhante. |
| Revisado | Alta | `/solicitacoes-compra` | `app/solicitacoes-compra/page.tsx`, `components/purchases/purchase-requests-portal.tsx` | A tela passou a ler a observação do cabeçalho sincronizado e a observação individual de cada insumo. Quando há conteúdo, um indicador com tooltip aparece na lista, no cabeçalho e junto ao item; especificação e observação permanecem separadas e ambas são preservadas no CSV. A listagem agora cruza `supplier_quote_request_origins` com as cotações existentes e mostra separadamente o andamento real da cotação: não iniciada, uma cotação ou múltiplas cotações vinculadas à solicitação. |
| Revisado | Alta | `/kanban-compras` | `app/kanban-compras/page.tsx`, `components/purchases/purchase-project-kanban.tsx`, `features/purchases/project-kanban-data.ts`, `lib/purchase-project-kanban.ts`, `app/api/purchases/project-kanban/route.ts` | Criado Kanban gerencial com projetos/obras cadastrados localmente, fora do Sienge. Solicitações do espelho local podem ser vinculadas e desvinculadas em cada projeto, com vínculo único para impedir dupla contagem. Etapas são dinâmicas e ordenáveis; o modal permite editar nomes e definir marcadores exclusivos `Inicial` e `Finalizada`, classificando projetos e solicitações como backlog, iniciados ou concluídos e impedindo uma ordem inválida. Cada solicitação vinculada possui etapa própria e usa as mesmas colunas do projeto; vínculos existentes são migrados para a etapa atual do projeto e novos entram na primeira coluna. A etapa do vínculo pode ser alterada tanto em `Gerenciar vínculos` quanto diretamente no cabeçalho da solicitação na visão gerencial consolidada. Nessa visão, todas as solicitações abrem recolhidas e funcionam como acordeão exclusivo; ao expandir uma, o painel lateral passa a mostrar somente suas cotações e desaparece quando ela é recolhida. Solicitações e cotações expandidas renderizam todos os itens sem limite; a janela passou a usar uma única rolagem vertical e descrições quebram linha sem reticências, eliminando o recorte causado pelas rolagens internas. A tipografia operacional foi revista para eliminar textos de 7–9px: rótulos partem de 10px e dados principais usam 11–13px, com linhas redimensionadas para evitar sobreposição. Os indicadores `S.C.`, `Itens` e `Cotações` dos cartões possuem marcação e seletores CSS próprios, três colunas estáveis e quebra interna, sem ultrapassar os limites do cartão. Etapas marcadas ou com projetos/solicitações não podem ser excluídas. Cartões podem ser movidos por drag-and-drop ou seletor, com prévia junto ao cursor e posição exata persistida. O painel geral de pendências cruza o vínculo local cotação → solicitação com solicitação → projeto e permite resolver casos incompletos localmente. A tela possui busca, estados vazios, layout responsivo, i18n e permissão própria `screen.kanban-compras`; o conteúdo usa toda a largura disponível e o quadro mantém rolagem horizontal em telas menores. Nenhuma ação grava no Sienge. |
| Revisado | Alta | `/cotacoes` | `app/cotacoes/page.tsx`, `components/purchases/quotations-portal.tsx`, `components/purchases/quotations/*`, `app/api/sienge/purchase-quotations/route.ts` | Revisado: a tela lê o espelho local, mantém filtros/status/exportação CSV, permite selecionar quais itens da solicitação entram na cotação Sienge e chama o Sienge somente nos botões de preparo/confirmação de criação da cotação. A solicitação que originou a cotação é gravada localmente em `supplier_quote_request_origins`, aparece como `SC-N` na lista e também entra na pesquisa e no CSV; registros antigos sem vínculo local ficam explicitamente como não identificados. |
| Revisado | Alta | `/cotacoes/[id]` | `components/purchases/quotation-detail/index.tsx`, `components/purchases/quotation-detail/types.ts`, `components/purchases/quotation-detail/tabs/*`, `app/api/sienge/purchase-quotations/route.ts`, `app/api/sienge/suppliers/route.ts`, `app/globals.css` | Revisado: o detalhe mantém 10 abas separadas por arquivo, com ações Sienge concentradas nas abas Sienge, Respostas, Aprovar, Mapa e Cadastros; Mapa/Aprovar seguem em layout de decisão. Adicionado botão "Mapa em PDF" na aba Mapa (independente do Sienge): modal escolhe todos os itens, itens marcados manualmente ou só os com proposta, e gera relatório em `/cotacoes/[id]/mapa-pdf` com layout próprio em matriz horizontal (cada item aparece em uma linha, fornecedores aparecem uma vez como colunas e cada célula mostra preço/total/quantidade/prazo; a coluna final `Melhor cesta` mostra apenas o menor valor unitário e o total consolidado no rodapé), verde reservado só para as células de melhor preço, sem preenchimento verde em cabeçalho ou seleção do modal para não confundir com o destaque de melhor preço, e impressão/salvar PDF pelo navegador. Extraída a lógica do mapa item a item para `buildItemComparison` em `helpers.ts`, reaproveitada pelo client (aba Mapa) e pelo server (relatório em PDF), testado manualmente na cotação #14 nos três modos (todos, com proposta, selecionados). Corrigido erro `negotiation.supplier.invalid.id` ("Fornecedor não está associado a cotação") ao enviar negociação para fornecedor que nunca foi incluído em nenhum item da cotação no Sienge: a rota agora inclui o fornecedor em cada item negociado antes de criar a negociação, sem bloquear o fluxo se essa associação já existir. Corrigido também o número de negociação extraído do `location` do Sienge após criar: a função genérica usada antes pegava o ID da cotação em vez do número da negociação (mesmo padrão numérico aparece antes na URL), causando erro 422/403 nos passos seguintes; criada `parseTrailingIdFromLocation` específica para esse caso. Adicionado botão "Sincronizar cotação com o Sienge" na aba Respostas: envia a negociação de todas as respostas ativas com fornecedor cadastrado de uma vez (par Conferir/Sincronizar, mesmo dry-run/confirmação dos outros botões), sem autorizar nada, reaproveitando a rota `send-negotiation` já corrigida. Adicionado retry único (600ms) quando o Sienge devolve 500 sem detalhe ao gravar um item da negociação (falha transitória observada logo após criar a negociação, com os outros itens da mesma negociação gravando normalmente); erros 4xx de validação continuam sem retry. Escritas confirmadas agora atualizam o espelho local de cotações e as telas na sequência (a cotação criada aparecia só depois de Atualizar Compras manual), e cotações excluídas no Sienge deixaram de ficar como fantasmas no espelho local. |
| Revisado | Alta | `/cotacoes/[id]/solicitacao-fornecedor` | `app/cotacoes/[id]/solicitacao-fornecedor/page.tsx`, `components/purchases/quotation-detail/tabs/insumos-tab.tsx`, `features/quotations/data.ts`, `app/globals.css` | A aba Insumos abre uma solicitação de proposta em A4 horizontal para envio manual ao fornecedor, com itens, quantidades, especificações, observações e campos comerciais em branco. Quando o espelho da cotação não contém a observação individual, o carregamento cruza os itens com a solicitação local de origem por produto, especificação, quantidade e unidade; a observação recuperada aparece na linha do item tanto no documento completo quanto no resumido. O relatório é protegido por sessão, não mostra preços ou decisões internas e permite imprimir ou salvar em PDF. |
| Revisado | Alta | `/cotacoes/[id]/solicitacao-fornecedor-resumida` | `app/cotacoes/[id]/solicitacao-fornecedor-resumida/page.tsx`, `components/purchases/quotation-detail/tabs/insumos-tab.tsx`, `app/globals.css` | Segunda opção de documento na aba Insumos: versão resumida em A4 vertical somente com número da solicitação, comprador e lista compacta de itens. Não contém campos de resposta, preços ou decisões internas e mantém a versão completa disponível separadamente. |
| Revisado | Alta | `/portal-cotacao/[token]` | `app/portal-cotacao/[token]/page.tsx`, `components/suppliers/*`, `lib/supplier-quote-portal.ts` | Revisado: portal público sem login, com validação de e-mail/telefone, frete obrigatório sem opção pré-selecionada, desconto à vista por porcentagem ou valor manual sem seleção inicial, itens parciais em amarelo, itens não cotados separados no detalhe final/impressão, proposta enviada apenas para consulta e campos obrigatórios destacados em vermelho ao tentar avançar etapa incompleta. A observação individual do insumo aparece ao fornecedor como orientação somente leitura no preenchimento e na conferência, separada do campo `Sua observação (opcional)`; a observação geral da solicitação não é passada ao componente público e continua restrita ao operador. |
...
| Revisado | Alta | `/titulos` | `app/titulos/page.tsx`, `features/titles/data.ts`, `features/financeiro/sienge-data.ts`, `components/titles/titles-explorer.tsx` | Criada consulta unificada e paginada de títulos a pagar e receber. A busca cobre número, documento, empresa, credor/cliente, origem e conteúdo da observação; filtros adicionais tratam tipo, situação, empresa, origem, observação, período e ordenação. Observações a pagar usam o cabeçalho mais recente de `/v1/bills` com complemento de `ecpgtitulo.deobservacao` do dump; a atualização passou a paginar todos os cabeçalhos, corrigindo o limite anterior de 200 registros. A tela lê somente os bancos locais e sinaliza que a integração pública de recebíveis não fornece observação do título. |
| Revisado | Média | `/contas-pagar` | `app/contas-pagar/page.tsx`, `features/payables-schedule/data.ts`, `features/payables-recurring/data.ts`, `components/payables/recurring-payables-panel.tsx` | Revisado: agenda segue lendo SQLite local e o atalho direto para `Novo lançamento` foi removido para não misturar consulta com escrita no Sienge. Adicionado painel "Pagamentos recorrentes" com situação "Sem futuro" ou "Passado + futuro", janela histórica de 3/6/9/12 meses ou no ano, último e próximo vencimentos e botão de impressão isolada. |
...
- A navegacao foi reorganizada em areas com submenus recolhiveis e icones consistentes; a sidebar abre compacta e com grupos fechados, expande sobre o conteudo ao receber foco ou mouse e pode ser fixada aberta pelo usuario.
...
### Etapa 18 revisada - Últimos ajustes de cotações e espelho local

Revisado nesta etapa:

- `/cotacoes`: criação a partir de solicitação agora permite selecionar itens por modal antes do dry-run e da gravação no Sienge; sem item selecionado, preparar/gravar ficam bloqueados.
- `/api/sienge/purchase-quotations`: gravações em sequência usam pausa curta e backoff para reduzir erro 429/5xx em loops de itens, fornecedores e negociação.
- `/cotacoes/[id]`: aba Respostas ganhou sincronização em lote das respostas ativas com fornecedor cadastrado, sem autorizar a negociação.
- `/cotacoes/[id]/mapa-pdf`: relatório ganhou orientação horizontal/vertical e layout horizontal mais compacto para impressão.
- Espelho local: gravações confirmadas no Sienge atualizam `/bulk-data/v1/purchase-quotations` e as telas fazem `router.refresh()`; cotações excluídas no Sienge deixam de ficar como fantasmas quando a carga é completa.
- Operação local de 10/07/2026: a base SQLite local de cotações foi limpa para reimportação, removendo apenas `/bulk-data/v1/purchase-quotations` de `purchases.sqlite` e `00-cache.sqlite`.

Resultado: o fluxo recente de cotações ficou documentado desde a seleção parcial de itens até a sincronização com o Sienge e a atualização do espelho local.
