...
- Ao abrir um cartão, a tela entra em uma visão gerencial consolidada, sem navegar para Solicitações ou Cotações. Ela mostra etapa, encerramento, cobertura, totais de solicitações, itens, cotações, fornecedores e respostas, além da quantidade de vínculos em backlog, iniciados e concluídos. Cada solicitação exibe sua etapa e expande itens, quantidades e observações; cada cotação expande itens, fornecedores, propostas, menor valor e seleção. Os dados vêm somente dos espelhos SQLite e das respostas locais do portal. Edição e gestão de vínculos são responsabilidades separadas e só aparecem ao clicar nos respectivos botões do cabeçalho do detalhe.
