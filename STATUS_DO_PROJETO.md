...
- Novo painel "Recorrentes sem lançamento futuro" (`features/payables-recurring/data.ts`, `components/payables/recurring-payables-panel.tsx`), pensado pra achar pagamentos recorrentes que ninguém cadastrou como título futuro. A recorrência é medida por título (`bulk_outcome_installments`, campo `dueDate`), não por baixa — um título conta pro mês do vencimento esteja pago ou em aberto (o pedido original era olhar pagamentos com baixa, mas foi corrigido pra títulos, já que baixa só existe pra quem já foi pago). Seletor de janela (3, 6, 9, 12 meses ou "No ano") calculado no cliente a partir de 12 meses já carregados no servidor, sem nova consulta por clique; total e média também são recalculados somente com os meses selecionados. Um fornecedor entra na lista quando tem título em todos os meses da janela escolhida e não tem nenhum título com vencimento futuro cadastrado. Testado contra consulta SQL independente batendo os mesmos 10 fornecedores para a janela de 3 meses.
