...
- Ao abrir um cartão, a tela entra em uma visão gerencial consolidada, sem navegar para Solicitações ou Cotações. Ela mostra etapa, encerramento, cobertura, totais de solicitações, itens, cotações, fornecedores e respostas, além da quantidade de vínculos em backlog, iniciados e concluídos. Todas as solicitações começam recolhidas e somente uma pode ficar expandida por vez. A solicitação aberta exibe itens, quantidades e observações; o painel de cotações surge ao lado somente nesse momento, filtrado para a solicitação ativa, troca de conteúdo ao abrir outra e desaparece ao recolhê-la. A etapa de cada solicitação também pode ser alterada diretamente nessa visão. Os dados vêm somente dos espelhos SQLite e das respostas locais do portal. Edição do projeto e gestão de inclusão/remoção de vínculos continuam separadas nos respectivos botões do cabeçalho.
