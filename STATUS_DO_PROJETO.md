...
- As colunas são dinâmicas: podem ser criadas, renomeadas, reordenadas e excluídas. Em `Etapas do Kanban`, cada nome pode ser editado e salvo por um botão identificado ou pela tecla Enter; o campo é sincronizado com o valor confirmado pelo servidor. Uma etapa com projetos não pode ser excluída até que seus cartões sejam movidos.
- Cada projeto pode ter uma previsão de encerramento. O cartão mostra a data e recebe alerta quando ela vence fora da etapa padrão `Concluído`; o resumo do quadro conta os encerramentos em atraso.
...
- Ao abrir um cartão, a tela entra em uma visão gerencial consolidada, sem navegar para Solicitações ou Cotações. Ela mostra etapa, encerramento, cobertura, totais de solicitações, itens, cotações, fornecedores e respostas; cada solicitação expande seus itens, quantidades e observações, e cada cotação expande itens, fornecedores, propostas, menor valor e seleção. Os dados vêm somente dos espelhos SQLite e das respostas locais do portal. Edição e gestão de vínculos são responsabilidades separadas e só aparecem ao clicar nos respectivos botões do cabeçalho do detalhe.
