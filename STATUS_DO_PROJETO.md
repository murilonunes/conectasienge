...
- `/configuracoes` agora possui a preferência opcional `Data de corte de compras`. Solicitações e cotações com data anterior ficam ocultas em `/solicitacoes-compra`, `/cotacoes`, seus detalhes e `/kanban-compras`; a própria data continua incluída. O filtro é aplicado antes de montar listas, indicadores, contagens e vínculos, inclusive nas solicitações manuais do navegador e nas respostas posteriores da API do Kanban. Os dados locais e os vínculos persistidos não são apagados, e registros sem data confiável permanecem visíveis. Campo vazio desativa o corte.
- Foi criada a tela `/kanban-compras`, no menu Compras, para cadastrar projetos/obras gerenciais fora do Sienge e reunir neles solicitações de compra já existentes no espelho local. Cada solicitação pode pertencer a somente um projeto por vez, possui etapa própria dentro do projeto e pode ser vinculada, movida ou desvinculada sem alterar o registro de origem. As mesmas etapas do quadro são usadas pelos projetos e seus vínculos; os marcadores exclusivos `Inicial` e `Finalizada` definem o que ainda está em backlog, o que foi iniciado e o que foi concluído. O painel `Pendências de vínculo` identifica cotações sem solicitação de origem e solicitações de origem ainda fora de projetos, permitindo resolver os dois vínculos sem gravar no Sienge. Projetos, etapas, prazos e vínculos ficam no SQLite local `purchase-project-kanban.sqlite`, com permissão própria `screen.kanban-compras`.
...
- As solicitações disponíveis são lidas do espelho local de Compras e podem ser vinculadas ou desvinculadas no detalhe do projeto. A restrição única por número de solicitação impede que a mesma `SC-N` seja contada em dois projetos. Cada vínculo possui sua própria etapa, selecionada em `Gerenciar vínculos`, reutilizando exatamente as colunas do projeto.
- As colunas são dinâmicas: podem ser criadas, renomeadas e reordenadas. Em `Etapas do Kanban`, cada nome pode ser salvo por botão ou Enter, e os controles `Inicial` e `Finalizada` marcam uma única coluna de cada tipo. Colunas anteriores à inicial são backlog, da inicial até antes da finalizada são trabalho iniciado, e a finalizada e as seguintes contam como concluídas. A ordem não permite inverter esses marcos; uma etapa marcada, com projetos ou com solicitações vinculadas não pode ser excluída até que o conteúdo ou o marcador seja movido.
- Cada projeto pode ter uma previsão de encerramento. O cartão mostra a data e recebe alerta quando ela vence antes da etapa marcada como `Finalizada`; o resumo do quadro separa backlog, projetos iniciados, projetos concluídos e encerramentos em atraso.
- O cartão consolida quantidade de solicitações, itens, cotações relacionadas e percentual de solicitações com cotação. Os indicadores `S.C.`, `Itens` e `Cotações` usam células de largura estável, com rótulos próprios que quebram dentro do limite sem ultrapassar o cartão. Cada solicitação mostra separadamente quantas cotações possui e os respectivos números. A troca de etapa funciona por arrastar e soltar, com prévia do cartão junto ao cursor, realce da coluna de destino e marcador entre os cartões para escolher a posição exata no início, no meio ou no fim da etapa. A ordem também pode ser alterada dentro da própria coluna, fica persistida no SQLite local e aparece imediatamente enquanto o servidor confirma a gravação. O seletor continua disponível como alternativa para telas sem drag-and-drop. A tela usa toda a largura disponível do conteúdo, mantém textos operacionais com tamanho legível e amplia as colunas até a altura útil da janela; em telas menores, preserva a largura operacional dos cartões com rolagem horizontal.
- Ao abrir um cartão, a tela entra em uma visão gerencial consolidada, sem navegar para Solicitações ou Cotações. Ela mostra etapa, encerramento, cobertura, totais de solicitações, itens, cotações, fornecedores e respostas, além da quantidade de vínculos em backlog, iniciados e concluídos. Todas as solicitações começam recolhidas e somente uma pode ficar expandida por vez. A solicitação aberta exibe todos os itens, quantidades, descrições completas e observações; o painel de cotações surge ao lado somente nesse momento, filtrado para a solicitação ativa e também sem limitar os itens. A janela usa uma única rolagem vertical para não esconder o final das listas, troca o conteúdo ao abrir outra solicitação e remove o painel ao recolhê-la. A etapa de cada solicitação também pode ser alterada diretamente nessa visão. Botões que apenas encerram os modais usam o texto `Fechar`, ícone de X e destaque laranja, sem sugerir que executam uma conclusão. Os dados vêm somente dos espelhos SQLite e das respostas locais do portal. Edição do projeto e gestão de inclusão/remoção de vínculos continuam separadas nos respectivos botões do cabeçalho.
- O painel `Pendências de vínculo` cruza `supplier_quote_request_origins` com o Kanban. Ele separa cotações sem solicitação de origem das cotações cuja solicitação ainda não pertence a projeto; a primeira associação grava somente o vínculo local cotação → solicitação e a segunda usa o vínculo local solicitação → projeto.
- O arquivo `.sienge-data/purchase-project-kanban.sqlite` guarda somente projetos, etapas, marcadores de início/finalização e vínculos com suas etapas. Os dados das solicitações e cotações continuam sendo lidos das bases locais já sincronizadas. Na migração, `Planejamento` vira a etapa inicial, `Concluído` vira a finalizada e vínculos já existentes herdam a etapa atual do projeto; novas solicitações entram na primeira coluna, antes do início, quando houver backlog.
...
- A mesma aba oferece `PDF resumido`, em `/cotacoes/[id]/solicitacao-fornecedor-resumida`, que preserva apenas o número da solicitação, o comprador e a listagem compacta de itens. Essa alternativa não contém campos de resposta e também pode ser impressa ou salva em PDF.
...
- O portal público `/portal-cotacao/[token]` permite ao fornecedor informar, item a item: se atende, preço unitário, quantidade, prazo diferente do pedido e sua própria observação; preço zero informado é tratado como valor válido. Quando o insumo possui uma observação do comprador, ela aparece em um bloco somente leitura no cartão do item e na conferência final. A observação geral da solicitação permanece interna e não é enviada ao portal público.
...
- O Kanban de compras ganhou a visão de impressão gerencial em `/kanban-compras/impressao`: o botão no quadro abre um seletor de etapas, permite escolher quais colunas entram no papel e gera um relatório sem o menu do sistema, agrupado por etapa e projeto. O relatório inclui todas as S.C. visíveis pela data de corte, status da solicitação, etapa interna, datas, solicitante, quantidade de itens, cotações relacionadas com status e observações da solicitação e dos itens.
...
- Novo painel "Pagamentos recorrentes" (`features/payables-recurring/data.ts`, `components/payables/recurring-payables-panel.tsx`), pensado pra achar pagamentos recorrentes e conferir se os próximos títulos já foram cadastrados. A recorrência é medida por título (`bulk_outcome_installments`, campo `dueDate`), não por baixa — um título conta pro mês do vencimento esteja pago ou em aberto. O seletor de situação alterna entre fornecedores recorrentes sem futuro e fornecedores que têm títulos no passado e também no futuro, mostrando último vencimento histórico e próximo vencimento futuro. O seletor de janela (3, 6, 9, 12 meses ou "No ano") é calculado no cliente a partir de 12 meses carregados no servidor; total e média são recalculados somente com os meses selecionados. Testado contra consulta SQL independente batendo os mesmos 10 fornecedores sem futuro para a janela de 3 meses.
- Botão "Imprimir este painel" (`components/ui/print-panel-button.tsx`, reutilizável) imprime só o painel, escondendo menu lateral e resto da tela via classe `printing-single-panel` no `<html>` (adicionada ao clicar, removida no evento `afterprint`) combinada com atributo `data-print-panel` no painel-alvo. CSS de impressão reduz padding/fonte da tabela pra caber mais fornecedores por página.
...
- `/compras` virou o portal "Suprimentos": a visão geral mostra a cadeia de compras com filtro server-side por período. `Mês atual` vem marcado por padrão e o histórico adicional aceita +1, +2, +3, +6, +12 meses ou Tudo pelos parâmetros `mesAtual` e `historico`. O mesmo recorte alimenta os cinco cards, o funil Solicitações > Cotações > Pedidos > Notas, os quatro painéis de ação, os rankings e os gráficos. Solicitações para cotar têm atalho para `/cotacoes?solicitacao=N` e detecção de cotação provável por sobreposição de insumos. Lógica em `features/purchases/supply-overview.ts`; status de cotação reaproveita `quotationSummary` de `features/quotations/data.ts`.
- Nova sincronização de situação das solicitações (`features/purchases/request-headers.ts`): o espelho em lista (`/v1/purchase-requests/all/items`) não diz se a solicitação já foi atendida, então o job de Atualizar Compras passou a consultar o endpoint individual `/v1/purchase-requests/{id}` (único que traz `status` PENDING/FULLY_ATTENDED, `requestDate` e solicitante) e grava na tabela `purchase_request_headers` de `purchases.sqlite`. A carga é incremental: solicitações em status final (atendida/cancelada/reprovada) não são consultadas de novo, só novas e em aberto; há pausa e retry entre chamadas para respeitar o limite de taxa do Sienge. Sem essa sincronização o funil tratava as ~950 solicitações históricas como abertas; a tela avisa quando a situação ainda não foi sincronizada.
...
- Fontes usadas: pedidos de compra, notas/faturas de compra, itens de solicitacoes, cotacoes de compra e situação individual das solicitações.
